package com.example.geo.demo.utils;
...
public class RedisUtils {
     ...
     * @return
     */
    public Long redisGeoAdd(String key, double lng, double lat, String name) {
        Long addedNum = redisTemplate.opsForGeo().add(key, new Point(lng, lat), name);//params: key, Point(经度, 纬度), 地方名称
        return addedNum;
    }
     ...
     * @return
     */
    public GeoResults<RedisGeoCommands.GeoLocation<String>> redisNearByXY(String key, double lng, double lat, Integer distance, Integer count) {
        Circle circle = new Circle(new Point(lng, lat), new Distance(distance, Metrics.KILOMETERS));//Point(经度, 纬度) Distance(距离量, 距离单位)
